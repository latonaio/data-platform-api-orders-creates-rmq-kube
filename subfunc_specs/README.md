...
| Target / Processing Type     | Key(s)                                       | 
| ---------------------------- | -------------------------------------------- | 
| 下記Field Searchedと同一 <br> / Get and Hold               | PartnerFunctionBusinessPartner(2-1で取得)    | 
| Table Searched               | Name of the Table                            | 
...
| business_partner                                        | Buyer | PartnerCounter | PartnerFunction | PartnerFunctionBusinessPartner | BusinessPartnerName  | 
| ------------------------------------------------------- | ----- | -------------- | --------------- | ------------------------------ | -------------------- | 
| (※FullName, Language, Currency, AddressIDの例示は省略) | 
...
| Target / Processing Type                                               | Key(s)                                                                        | 
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------- | 
| 下記Field Searchedと同一 <br>  / Get and Hold                                                         | BusinessPartner=business_partner(入力ファイル) <br> Customer=Buyer(入力ファイル)または <br> Supplier=Seller(入力ファイル) <br> PartnerCounter(2-1-1で取得)                                            | 
| Table Searched                                                         | Name of the Table                                                             | 
...
| Target / Processing Type                                   | Key(s)                                                             | 
| ---------------------------------------------------------- | ------------------------------------------------------------------ | 
| 下記Field Searchedと同一 <br> / Get and Hold                                             | BusinessPartner=business_partner(入力ファイル) <br> Customer=Buyer(入力ファイル)または、 <br> Supplier=Seller(入力ファイル)  <br> PartnerCounter(2-1-1で取得) <br>  PartnerFunction(2-1-1で取得) <br> PartnerFunctionBusinessPartner(2-1-1で取得)                | 
| Table Searched                                             | Name of the Table                                                  | 
...
| Property                         | Description                                                                                                                                                  | EC  | 
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --- | 
| StockConfirmationPartnerFunction | 在庫確認を行う取引先機能。ヘッダパートナプラントから設定される。必要に応じて変更する。（PRODUCT_STOCK_AVAIRABILITY_CHECK_SRVを利用、機能はオプション）       | 
| 参照の場合は変更できない。       | ✔                                                                                                                                                           | 
| StockConfirmationBusinessPartner | 在庫確認を行うビジネスパートナ。ヘッダパートナプラントから設定される。必要に応じて変更する。（PRODUCT_STOCK_AVAIRABILITY_CHECK_SRVを利用、機能はオプション） | 
| 参照の場合は変更できない。       | ✔                                                                                                                                                           | 
...
| Batch Master Record SQL Batch Data                                                                       | data_platform_batch_master_record_batch_data        | 
| Field Created                                                                                            | Data Type / Number of Digits                        | 
| Product <br> BusinessPartner  <br> Plant  <br> Batch <br> CountryOfOrigin(4-2-1で取得) <br> ValidityStartDate(入力ファイルのProductionPlantBatchValidityStartDateまたは、ブランクの場合システム日付) <br> ManufactureDate(ブランク) <br> CreationDateTime(システム日付時刻) <br> LastChangeDateTime(システム日付時刻) <br> IsMarkedForDeletion(false)                                                                               | string(varchar) / 40  int / 12 <br> string(varchar) / 4 <br> string(varchar) / 10 <br> string(varchar) / 3  <br> date <br> date <br> datetime <br> datetime <br> bool (tinyint) / 1                                                                                       | 
| Single Record or Array                                                                                   | Memo                                                | 
| Array                                                                                                    | data_platform_batch_master_record_createsを用いる。 | 
...
| Table Searched                                             | Name of the Function                              | 
| None                                                       | data_platform_function_product_availability_check | 
| Field Searched                                             | Data Type / Number of Digits                      | 
| Product  <br> BusinessPartner  <br> Plant <br> Batch <br> BatchValidityEndDate <br> ProductStockAvailabilityDate  <br> AvailableProductStock                                      | string(varchar) / 40 <br> int / 12   string(varchar) / 4  string(varchar) / 10  <br> date <br> date  <br> float / 15                                                 | 
| Single Record or Array                                     | Memo                                              | 
| Array                                                      |                                                   | 
...
| ConfirmedDeliveryDate                                                                            | 5-28-2-2で取得したProductStockAvailabilityDate                                                                 | 
| ※確認された在庫が無い場合、API処理の結果により、RequestedDeliveryDateと同じ日付がセットされる。 | 
| OrderQuantityInBaseUnit                                                                          | 本処理(5-28-2)にて保持済みのOrderQuantityInBaseUnit。                                                          | 
| ConfdOrderQtyByPDTAvailCheck                                                                     | 5-28-2-2で取得したAvailableProductStock                                                                        | 
| ※確認された在庫が無い場合、API処理の結果により、ゼロがセットされる。                            | 
...
5-36-2. 5-36-1の検索結果が0件であった場合、ItemDeliveryStatus
に”NP”(未入出荷)をセットする。検索結果が1件以上であり、かつ、全ての検索結果値がfalseの場合は”NP”(未入出荷)をセットする。検索結果が1件以上であり、かつ、 全ての検索結果値がtrueの場合は”CL”(入出荷完了済)をセットする。それ以外の場合は”PP”(部分出荷完了済)をセットする。
...
| Table Searched           | Name of the Table            | 
| None                     | None                         | 
| Field Arranged           | Data Type / Number of Digits | 
...
Target / Processing Type	Key(s)
下記Field Searchedと同一
/ Get and Set	[AddressID]
[ValidityEndDate≧システム日付]
| Table Searched           | Name of the Table                  | 
| ------------------------ | ---------------------------------- | 
...
| Target / Processing Type              | Key(s)                                    | 
| ------------------------------------- | ----------------------------------------- | 
| 下記Field Searchedと同一 <br> / Get and Set       | ReferenceDocument / ReferenceDocumentItem | 
| Table Searched                        | Name of the Table                         | 
...
| Field Created                   | Data Type / Number of Digits | Specification                            | 
| ------------------------------- | ---------------------------- | ---------------------------------------- | 
